...
class Parser:
    ...
    def is_command(self, index):
        return not (not (self.input_lines[index]) or
                    self.input_lines[index].strip()[0] == '/')
    ...
    def command_type(self) -> str:
        ...
        """
        # Your code goes here!
        arithmetic_set = {"add", "sub", "neg", "eq", "gt", "lt", "and", "or",
                          "not", "shiftleft", "shiftright"}
        other_ops = {'push': "C_PUSH", 'pop': 'C_POP', 'label': 'C_LABEL',
        ...
        if arg1 in arithmetic_set:
            return 'C_ARITHMETIC'
        if arg1 in other_ops:
            return other_ops[arg1]
